/* ----------------------------------------------------------------------
   ...
   $Source: /usr/export/cvsroot/deapi/c++/main/dealingtools/orc/deapi/Message.cc,v $
   $Revision: 1.1 $

   ---------------------------------------------------------------------- */

static char rcsid[] = "$Id: Message.cc,v 1.1 2004/06/03 09:08:12 john Exp $";
...
namespace deapi {
  ...
  Node::Node(Message *message) : msg(message) {
  }
  bool Node::IsMsg() {
    return (msg != NULL);
  }
  ...
  Message *Message::Parse(std::istreambuf_iterator<char>& inpos) {
	...
	case '|':
	  ...
	  if (!buf.empty()) {
	    insert(key, new Node(buf)); // trim buf!
	    lookingForKey = true;
	    buf.clear();
	  }
	  break;
